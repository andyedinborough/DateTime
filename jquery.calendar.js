(function (window, document, $, datetime, undefined) {
    if (!datetime) throw 'DateTime is required.';

    var defaults = {
        format: 'yyyy-MM-dd', time: false,
        buttons: {
            'Today': function (w, o) { w.dateselected(new Date()); },
            'Clear': function (w, o) { w.dateselected(''); }
        }
    }
    ...
    var calendar = $.calendar = function (options) {
        ...
        widget.dateselected = function (date) {
            ...
            return this.trigger({ type: 'day-selected', target: this, date: options.date, options: options,
                formatDate: function (format) {
                    format = format || options.format;
                    return options.date ? datetime.format(options.date, format, options) : undefined;
                }
            });
        }
        ...
        widget.goto = function (date, select) {
            ...
            if (select) options.date = date == '' ? undefined : datetime(options.dateRender);
            this.render();
            return this;
        }
    ...
    $.fn.datetimepicker = function (o) {
        return this.each(function () {
            var self = $(this), options = $.extend({ time: true, format: self.data('format') || 'yyyy-MM-dd HH:mm:ss' }, o);
            self.datepicker(options);
        });
    }

    function build(options) {
        var date = options.dateRender || (options.dateRender = datetime(options.date)), html = '', days = new Date(date.year, date.month, 0).getDate(), first = datetime(date.year, date.month, 1),
            rows = Math.ceil((days + first.dayOfWeek) / 7);

        html = '<div class="calender-header"><div><span class="action year-prev">&laquo;&laquo;</span><span class="action month-prev">&laquo;</span><span class="month-name">'
            + datetime.defaults.months[date.month - 1] + ' ' + date.year
            + '</span><span class="action month-next">&raquo;</span><span class="action year-next">&raquo;&raquo;</span></div><div>';
        for (var i = 0; i < 7; i++) {
            html += '<span class="day day-name '
        ...
        for (var i = 0; i < rows; i++) {
            ...
            for (var j = 0; j < 7; j++) {
                ...
                html += ' class="day day-' + shortNames[dow]
                    + (dow > 0 && dow < 6 ? ' day-week' : ' day-weekend')
                    + (display ? ' action' : '')
                    + (options.date && day == options.date.day && date.month == options.date.month && date.year == options.date.year ? ' day-selected' : '')
                    + '">';
