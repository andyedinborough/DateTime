var DateTime = (function (window, document, undefined) {
    var datetime = function (lock) {
        var result = this;
        if (!this.isDate)
            result = new datetime(true);

        if (lock !== true)
            result.fromDate(getDateFromArguments(arguments));
    ...
    datetime.pretty = function (date) {
        ...
        var diff = (((new Date()).getTime() - date.getTime()) / 1000),
            ...
            pre = '', post = ' ago', future = diff < 0, oneday = 'Yesterday';
        ...
        return day_diff == 0 && (
                diff < 60 && (
                    (future ? '' : 'just ') + 'now')
                    || diff < 120 && (pre + "1 minute" + post)
                    || diff < 3600 && (pre + Math.floor(diff / 60) + " minutes" + post)
                    || diff < 7200 && (pre + "1 hour" + post)
                    || diff < 86400 && (pre + Math.floor(diff / 3600) + " hours" + post)
                )
            || day_diff == 1 && oneday
            || day_diff < 7 && (pre + day_diff + " days" + post)
            || day_diff < 31 && (pre + Math.ceil(day_diff / 7) + " weeks" + post);
    }

    function fill(number, prelen, plus) {
        var postlen = prelen < 0 ? -prelen : 0, pre = number < 0 ? '-' : (plus || '');
    ...
    var formats = datetime.formats = {
        'pretty': function (d, o) { return datetime.pretty(d); },
        'yyyy': function (d, o) { return d.year; },
        'MMMM': function (d, o) { return o.months[d.month - 1]; },
        'dddd': function (d, o) { return o.days[d.dayOfWeek]; },
        'zzzz': function (d, o) {
            return fill(Math.floor(d.utcOffset), 2, '+') + fill((d.utcOffset % 1) * 60, 2);
        },

        'MMM': function (d, o) { return o.months[d.month - 1].substr(0, 3); },
        'ddd': function (d, o) { return o.days[d.dayOfWeek].substr(0, 3); },
        'FFF': function (d, o) { return d.millisecond; },
        'fff': function (d, o) { return fill(d.millisecond, -3); },
        'zzz': function (d, o) {
            return fill(Math.floor(d.utcOffset), 2, '+') + ':' + fill((d.utcOffset % 1) * 60, 2);
        },

        'yy': function (d, o) { return d.year.toString().substr(2); },
        'MM': function (d, o) { return fill(d.month, 2); },
        'dd': function (d, o) { return fill(d.day, 2); },
        'HH': function (d, o) { return fill(d.hour, 2); },
        'hh': function (d, o) { return fill(d.hour == 0 ? 12 : d.hour - (d.hour > 12 ? 12 : 0), 2); },
        'mm': function (d, o) { return fill(d.minute, 2); },
        'ss': function (d, o) { return fill(d.second, 2); },
        'FF': function (d, o) { return Math.round(d.millisecond / 10); },
        'ff': function (d, o) { return fill(Math.round(d.millisecond / 10), -2); },
        'tt': function (d, o) { return d.hour >= 12 ? 'PM' : 'AM'; },
        'zz': function (d, o) { return fill(d.utcOffset, 2, '+'); },

        'M': function (d, o) { return d.month; },
        'd': function (d, o) { return d.day; },
        'H': function (d, o) { return d.hour; },
        'h': function (d, o) { return d.hour == 0 ? 12 : d.hour - (d.hour > 12 ? 12 : 0); },
        'm': function (d, o) { return d.minute; },
        's': function (d, o) { return d.second; },
        'F': function (d, o) { return Math.round(d.millisecond / 100); },
        't': function (d, o) { return d.hour >= 12 ? 'P' : 'A'; },
        'z': function (d, o) { return fill(d.utcOffset, 1, '+'); },
        'k': function (d, o) { return d.timezone.name; }

    }, format_keys = [], common_formats = datetime.common_formats = {
        'd': 'M/d/yyyy',
        'D': 'dddd, MMM d, yyyy',
    ...
    datetime.format = function (d, format) {
        ...
        if (format_keys.length == 0) {
            for (var i in formats) format_keys.push(i);
        }
